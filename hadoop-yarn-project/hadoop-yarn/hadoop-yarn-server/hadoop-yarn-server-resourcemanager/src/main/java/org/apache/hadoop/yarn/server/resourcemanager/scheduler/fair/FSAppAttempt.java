  ...
  private Resource assignContainer(FSSchedulerNode node, boolean reserved) {
    ...
    synchronized (this) {
      for (Priority priority : prioritiesToTry) {
        ...
        LOG.info("13 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue FSapp priority  "+priority);
        ...
        if (scheduler.isContinuousSchedulingEnabled()) {
          ...
          LOG.info("14 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue FSapp isContinuousSchedulingEnabled  "
                  +priority+",nodelocal "+
                  scheduler.getNodeLocalityDelayMs()+",racklocal "
                  +scheduler.getRackLocalityDelayMs()+", "
                  +scheduler.getClock().getTime());
        } else {
          ...
          LOG.info("14 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue FSapp isContinuousSchedulingEnabled  "
                  +priority+", NumCluster"+
                  scheduler.getNumClusterNodes()+",nodelocal "
                  +scheduler.getNodeLocalityThreshold()+", racklocal"
                  +scheduler.getRackLocalityThreshold());
        }
