...
public class QueuePlacementPolicy {
  ...
  private static final Log LOG = LogFactory.getLog(QueuePlacementPolicy.class);
  static {
  ...
  public String assignAppToQueue(String requestedQueue, String user)
    ...
    for (QueuePlacementRule rule : rules) {
      String queue = rule.assignAppToQueue(requestedQueue, user, groups,
          configuredQueues);
      LOG.info("4 YARN HOUSHITAO APP_ADDED addApplication  assignAppToQueue : "+queue+",configuredQueues,"+configuredQueues+",groups "+groups);
