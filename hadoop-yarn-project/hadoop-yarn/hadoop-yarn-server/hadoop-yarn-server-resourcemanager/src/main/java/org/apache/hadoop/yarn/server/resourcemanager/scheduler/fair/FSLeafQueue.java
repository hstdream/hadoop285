...
public class FSLeafQueue extends FSQueue {
  ...
  public Resource assignContainer(FSSchedulerNode node) {
    ...
    try {
      for (FSAppAttempt app : runnableApps) {
        ...
        LOG.info("10.1 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue FSapp pending  "+pending+", app "+app.getName());
    ...
    for (FSAppAttempt sched : pendingForResourceApps) {
      ...
      LOG.info("10 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue child FSAppAttempt "+sched.getName());
  ...
  public boolean canRunAppAM(Resource amResource) {
    ...
    LOG.info("11 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue canRunAppAM  "+maxAMShare);
    if (Math.abs(maxAMShare - -1.0f) < 0.0001) {
      LOG.info("12 YARN HOUSHITAO attemptScheduling start assignContainer LeafQueue canRunAppAM  "+maxAMShare);
      return true;
    }
